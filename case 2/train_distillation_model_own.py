...
class OwnModel(DistillModel):
    ...
    def fit(self, recordings) -> "LinearModel":
        ...
        optimizer = torch.optim.Adam(self.modell.parameters(), lr=0.001)
        
        X = torch.tensor(X).to(torch.float32)
        y = torch.tensor(y).to(torch.float32)
        batch_size = 10000
        cleanup = False
        if int(len(X) / batch_size) < len(X) / batch_size:
            cleanup = True
        for epoch in range(10):
            for batch_index in range(int(len(X) / batch_size)):
                ...
                loss = criterion(prediction, y_batch.unsqueeze(1))

                loss.backward()
                optimizer.step()

            if cleanup:
                X_batch = X[int(len(X) / batch_size) * batch_size: -1]
                y_batch = y[int(len(X) / batch_size) * batch_size: -1]
                optimizer.zero_grad()
    ...
    def predict(self, df) -> dict:
        ...
        for j in range(N_JOINTS):
            X = self._row_features(j, ti[:, j], q[:, j], qd[:, j],
                                           qdd[:, j], vel, acc)
            out[:, j] = self.modell(X)
        out[:, j]
        return {"actual_current": out}
...
def main():
    ...
    print("coefficients:")
    for name, c in zip(OwnModel.FEATURE_NAMES, coef):
